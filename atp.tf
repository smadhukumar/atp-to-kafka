resource "oci_database_autonomous_database" "LiveLab_ATP_primary_database" {
...
resource "oci_database_autonomous_database" "LiveLab_ATP_remote_database" {
  count                        = var.remote_data_guard_enabled ? 1 : 0
  provider                     = oci.standbyregion
  source                       = "CROSS_REGION_DATAGUARD"
  source_id                    = oci_database_autonomous_database.LiveLab_ATP_primary_database.id
  compartment_id               = oci_identity_compartment.LiveLab1Compartment.id
  cpu_core_count               = var.database_cpu_core_count
  data_storage_size_in_tbs     = var.database_data_storage_size_in_tbs
  db_name                      = var.database_db_name
